...
export class RecetaService {
  ...
  getReceta(id: number | string) {
    ...
    return withObserver(new Receta(receta.id, receta.iduser, receta.title, receta.abstract, receta.details, receta.thumbnail, receta.thumbnailbig));
  }
  ...
  saveReceta(receta: Receta, idusuario: number){
    if(receta.id){
      let oldReceta = this.recetas.filter(h => h.id === receta.id)[0];
      oldReceta.iduser = idusuario;
